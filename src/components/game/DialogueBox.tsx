...
export const DialogueBox: React.FC<DialogueBoxProps> = ({ dialogue, isLoading, onSpeakingChange }) => {
  ...
  return (
    <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-[min(95%,900px)] z-20">
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className="relative bg-white/95 backdrop-blur-md border-[4px] border-vibrant-pink rounded-[24px] p-8 shadow-[0_10px_30px_rgba(216,27,96,0.15)]"
        >
          {/* Speaker Name Tag */}
          <div className="absolute -top-5 left-10 bg-vibrant-pink text-white px-6 py-1.5 rounded-[15px] font-bold text-lg shadow-[0_4px_8px_rgba(0,0,0,0.1)]">
            {dialogue?.speaker || ' Neo'}
          </div>

          <div className="min-h-[60px] text-[#333] text-lg leading-relaxed font-sans">
            {isLoading ? (
